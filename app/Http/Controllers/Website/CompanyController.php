...
class CompanyController extends Controller
{
    ...
    // used this function for main page of company dashboard and it's statistics
    ...
    public function payPerJobPayment()
    {
        try {
            ...
            // midtrans snap token
            if (config('webdeveloper.midtrans_active') && config('webdeveloper.midtrans_merchat_id') && config('webdeveloper.midtrans_client_key') && config('webdeveloper.midtrans_server_key')) {
                $usd = $job_total_amount;
                $checkCurrency = Currency::where('code', 'IDR')->first();
                if ($usd && $checkCurrency) {
                    $fromRate = Currency::whereCode(config('webdeveloper.currency'))->first()->rate;
            ...
            // Flutterwave Amount
            if (config('webdeveloper.flw_public_key') && config('webdeveloper.flw_secret') && config('webdeveloper.flw_secret_hash') && config('webdeveloper.flw_active')) {
...
//            }
    ...
    public function plan()
    {
        try {
            ...
            $current_language_code = $current_language ? $current_language->code : config('webdeveloper.default_language');
