...
//            $query = Job::with('company.user', 'category', 'job_type:id,name')
    ...
    {
        if (config('webdeveloper.default_job_provider') == 'indeed' && config('webdeveloper.indeed_id')) {
            ...
            $q = $keywords;
            $l = $location ?? '';
            $limit = $limit ?? (config('webdeveloper.indeed_limit') ?? 10);
            $start = '';
            $end = '';
            $sort = 'date';
            $jt = '';
            ...
            $radius = '';
            $data = [
                'publisher' => config('webdeveloper.indeed_id'),
                'v' => 2,
                'format' => 'json',
                'q' => $q,
                'l' => $l,
    ...
    {
        if (config('webdeveloper.default_job_provider') == 'careerjet' && config('webdeveloper.careerjet_id')) {
            $keyword = $request->keyword ?? '';
            // $id = JobCategory::where('slug', $slug)->first();
            // $category_id = $id->id;
            // $category = $category_id ? JobCategory::whereId($category_id)->first() : '';
            ...
            $page = $page ?? 1;
            $pagesize = $pagesize ?? (config('webdeveloper.careerjet_limit') ?? 10);
            $result = $this->search([
                'keywords' => $keywords ?? '',
                'location' => $location ?? '',
                'page' => $page,
                'sort' => 'date',
                'pagesize' => $pagesize,
                'affid' => config('webdeveloper.careerjet_id'),
    ...
    {
        $locale = config('webdeveloper.careerjet_default_locale');
