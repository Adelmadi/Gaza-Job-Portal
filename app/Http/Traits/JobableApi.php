...
trait JobableApi
{
    ...
    {
        if (config('gazajobs.indeed_active') && config('gazajobs.indeed_id')) {
            ...
            $q = $keywords;
            $l = $location ?? '';
            $limit = $limit ?? (config('gazajobs.indeed_limit') ?? 10);
            $start = '';
            $end = '';
            $sort = 'date';
            $jt = '';
            ...
            $radius = '';
            $data = [
                'publisher' => config('gazajobs.indeed_id'),
                'v' => 2,
                'format' => 'json',
                'q' => $q,
                'l' => $l,
    ...
    {
        if (config('gazajobs.careerjet_active') && config('gazajobs.careerjet_id')) {
            $keyword = $request->keyword ?? '';
            $category = $request->category ? JobCategory::whereId($request->category)->first() : '';
            $role = $request->job_role ? JobRole::whereId($request->category)->first() : '';
            $keywords = $keyword ? $keyword : ($category ? $category->name : ($role ? $role->name : 'job'));
            ...
            $page = 1;
            $pagesize = $pagesize ?? (config('gazajobs.careerjet_limit') ?? 10);
            $result = $this->search([
                'keywords' => $keywords ?? '',
                'location' => $location ?? '',
                'page' => $page,
                'sort' => 'date',
                'pagesize' => $pagesize,
                'affid' => config('gazajobs.careerjet_id'),
            ]);
    ...
    {
        $locale = config('gazajobs.careerjet_default_locale');
